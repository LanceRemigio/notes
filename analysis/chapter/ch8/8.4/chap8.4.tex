...
\begin{enumerate}
    \item[(a)] Show that the improper integral \( \int_{ a }^{ \infty  } f  \) converges if and only if, for all \( \epsilon > 0 \), there exists \( M > a  \) such that whenever \( d > c \geq M  \) it follows that 
        ...
        \begin{proof}
        For the forwards direction, suppose that the improper integral \( \int_{ a }^{ \infty  } f  \) converges. Let \( \epsilon > 0  \). By assumption, we can find an \( M > a  \) such that whenever \(  d >  c \geq M  \), it follows that  
        \begin{align*}
            \Big| \int_{ a }^{ d } f  - L  \Big| &< \frac{ \epsilon  }{ 2  } \ \ \text{whenever } d \geq M > a,  \\
            \Big| \int_{ a }^{ c } f - L  \Big| &< \frac{ \epsilon  }{ 2  }  \ \ \text{whenever } c \geq M > a.
        \end{align*}
        Observe that 
        ...
        Now assume the converse. Let \( \epsilon > 0 \).  
        \end{proof}
...
\begin{enumerate}
    \item[(a)] Use the properties of \( e^{t} \) previously discussed to show 
        ...
        \end{proof}
    \item[(b)] Show
        \[  \int_{ 0  }^{ \infty  }  e^{-t} \  dt, \ \text{for all } \alpha > 0. \]
        \begin{proof}
