...
Suppose we have the following set \( \{ {f}_{k }(x): k \in \Z^{+} \}  \) and suppose we have the interval \( (a, \infty]   \). If the supremum of this set is in \( (a,\infty] \), then is each \( {f}_{k}(x) \) contained in \( (a,\infty ] \)
