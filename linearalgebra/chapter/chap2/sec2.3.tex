...
\end{theorem}
\begin{proof}
    Fix \( u \in V  \), and define the linear transformations \( f: F \to V  \) by \( f(a) = au  \) and \( g: F \to W  \) by \( g(a) = a T(u) \) for all \( a \in F  \). Let \( \alpha = \{ 1 \}  \) be the standard ordered bases for \( F  \). Note that \( g = T f  \). Identifying column vectors as matrices and using Theorem 2.11, we obtain 
\end{proof}
